...
cache_dir： 模型保存的本地路径，建议使用绝对路径（如 ）D:/Large Model/Qwen1.5

执行后，模型将被下载到 目录下D:/Large Model/Qwen1.5/qwen/Qwen1___5-0___5B-chat

三、模型推理测试
1. 测试代码
创建 文件，验证模型是否能正常加载和推理：web_demo.py
...
启动后访问 即可使用聊天界面http://127.0.0.1:8000

五、常见问题与注意事项
模型路径错误： 确保 与实际下载路径完全一致，Windows 下建议使用 避免转义字符问题DEFAULT_CKPT_PATHr"路径"
显存不足： 0.5B 模型占用显存较小，若仍提示 OOM，可添加 参数切换到 CPU 推理--cpu-only
端口占用： 修改 参数为其他未被占用的端口（如 8080、8081）--server-port
流式输出: 代码中使用 实现了打字机效果的流式回复TextIteratorStreamer
六、项目结构参考
Qwen1.5/
├── 1.ipynb    # 模型下载脚本
